/*
 ...
 Source Schema         : mini_box

 Target Server Type    : MySQL
 Target Server Version : 80018
 File Encoding         : 65001

 Date: 09/11/2020 09:11:02
*/
...
(
    ...
    `name`  varchar(40) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '版块名称，一般是【游戏名称】，如果不是【游戏名称】，就是【杂谈】',
...
(
    ...
    `title`       varchar(41) CHARACTER SET utf8 COLLATE utf8_general_ci   NOT NULL COMMENT '帖子标题',
    ...
    `create_date` timestamp(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '帖子发布日期',
    `update_date` timestamp(0)                                             NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '帖子修改日期',
...
(
    ...
    `type`    varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci   NOT NULL COMMENT '评论类型，TC为正常帖子下的评论，RC是回复其他用户的评论，GC是游戏下的评论',
    `tid`     bigint(11)                                               NULL COMMENT '帖子ID，如果type=TC，该字段必填',
    `uid`     bigint(11)                                               NULL COMMENT '帖子ID，如果type=RC，该字段必填',
    `gid`     bigint(11)                                               NULL COMMENT '帖子ID，如果type=GC，该字段必填',
    `score`   decimal(18, 2)                                           NULL COMMENT '用户对游戏的评分，如果type=GC，该字段必填',
...
(
    ...
    `name`         varchar(40) CHARACTER SET utf8 COLLATE utf8_general_ci   NOT NULL COMMENT '游戏中文名',
    ...
    `description`  varchar(4000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '游戏简介',
...
-- ----------------------------
-- Table structure for mb_like    使用Redis做点赞功能
-- ----------------------------
/*
DROP TABLE IF EXISTS `mb_like`;
CREATE TABLE `mb_like`
(
    `lid`     bigint(11)                                             NOT NULL AUTO_INCREMENT COMMENT '主键',
    `type`    varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '点赞类型，TL给帖子点赞，CL给用户评论点赞',
    `tid`     bigint(11)                                             NULL     DEFAULT NULL COMMENT '帖子ID，如果type=TL，该字段必填',
    `cid`     bigint(11)                                             NULL     DEFAULT NULL COMMENT '评论id，如果type=CL，该字段必填',
    `state`   int(1)                                                 NOT NULL DEFAULT 0 COMMENT '记录状态，0有效，1无效',
    `is_like` int(1)                                                 NOT NULL DEFAULT 0 COMMENT '是否点赞，0是，1否，第一次点赞时生成记录，第二次点击是取消点赞，则直接update该字段为1',
...
(
    ...
    `type`  varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci  NOT NULL COMMENT '图片类型，枚举，用户头像UP、帖子图片TP、评论图片CP、游戏图片GP',
    `link`  varchar(400) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '图片链接',
...
(
    `tid`      bigint(11)                                             NOT NULL AUTO_INCREMENT COMMENT '主键',
    `tag_name` varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '标签名称',
    `gid`      bigint(11)                                             NOT NULL COMMENT '关联的游戏ID',
    PRIMARY KEY (`tid`) USING BTREE
) ENGINE = InnoDB
  AUTO_INCREMENT = 10000
...
(
    ...
    `create_date`    timestamp(0)                                            NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '用户创建日期',
    `exp`            int(2)                                                  NOT NULL DEFAULT 0 COMMENT '经验，每次升级需要10经验，升级后该字段值update为0',
    `update_date`    timestamp(0)                                            NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0) COMMENT '字段更新时间，修改该条记录则自动更新这个字段',
    `state`          int(1)                                                  NOT NULL DEFAULT 0 COMMENT '状态，记录当前记录是否有效，0有效，1无效',
...
-- ----------------------------
-- Table structure for mb_user_follow  使用Redis做这个功能
-- ----------------------------
/*
...
(
    `fid`         bigint(11)   NOT NULL AUTO_INCREMENT COMMENT '主键',
    `uid`         bigint(11)   NOT NULL COMMENT '关联的用户ID',
    `follow_id`   bigint(11)   NOT NULL COMMENT '关注者的ID',
    `is_follow`   int(1)       NOT NULL COMMENT '是否关注，0是已关注，1是取关，如果点了关注，会生成一条记录，此时取关，只需update这条记录即可',
    `follow_date` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) COMMENT '关注时间（生成记录时间）',
    `state`       int(1)       NOT NULL DEFAULT 0 COMMENT '状态，记录该条状态是否有效,0有效，1无效',
